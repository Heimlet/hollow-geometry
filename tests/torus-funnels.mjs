...
import {ORBIT_SEEDS,orbitPoint,spiralGuide,torusFrameFromAnchors,SPIRAL_WINDOW} from '../js/torus-math.js';
import {goldenFunnelRadius,goldenFunnelCurvature,goldenFunnelBounds,funnelReveal,FUNNEL_EXTENT,spiralFunnelRadius,SPIRAL_FUNNEL_EXTENT} from '../js/torus-funnel-math.js';
...
// The second surface carries EVERY point of both guides, not just two rings.
assert.equal(SPIRAL_FUNNEL_EXTENT,PHI**SPIRAL_WINDOW.to);
for(const scale of [.2,1,17])for(const seed of ORBIT_SEEDS)for(let i=0;i<=1024;i++){
  const turn=SPIRAL_WINDOW.from+(SPIRAL_WINDOW.to-SPIRAL_WINDOW.from)*i/1024;
  const p=spiralGuide(seed,turn).map(x=>x*scale),radius=Math.hypot(p[0],p[1]);
  assert.ok(Math.abs(radius-spiralFunnelRadius(p[2]))/radius<1e-12,'Exact cones carry every mirrored guide in every scale');
}
...
for(const name of ['Upper spiral cone','Lower spiral cone']){
  const mesh=scene.getObjectByName(name),positions=mesh.geometry.attributes.position;
  for(let i=0;i<positions.count;i++){
    const radius=Math.hypot(positions.getX(i),positions.getY(i)),expected=spiralFunnelRadius(positions.getZ(i));
    assert.ok(Math.abs(radius-expected)/Math.max(1,expected)<2e-7,'Rendered cone vertices obey the same exact equation');
  }
}
...
const anchors=ORBIT_SEEDS.map(seed=>orbitPoint(seed,.6)),frame=torusFrameFromAnchors(anchors);
for(const rounded of [false,true])for(const spiral of [false,true]){
  surface.update('cosmos',.5,frame,anchors,{rounded,spiral});
  assert.equal(surface.group.visible,rounded||spiral);
  if(rounded||spiral){
    assert.equal(scene.getObjectByName('Rounded phi surfaces').visible,rounded);
    assert.equal(scene.getObjectByName('Exact spiral surfaces').visible,spiral);
  }
}
scene.traverse(o=>{if(o.geometry)assert.ok(geometry.has(o.geometry),'Buffers are reused during indefinite expansion and toggles');});
...
assert.ok([...geometry.values()].reduce((n,a)=>n+a.length/3,0)<28000,'Both surfaces fit a fixed mobile geometry budget');
