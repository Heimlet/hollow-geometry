...
Ручное вращение ставит фильм на паузу и передаёт кадрирование пользователю.
Ручная правка сцены завершает тур. В меню туров видны только звёзды, сохранённая
сцена возвращается при переходе в лабораторию. Тексты содержат ссылки на настройки.
...
Все 14 наборов проверок: `for f in tests/*.mjs; do node "$f" /path/to/three.module.mjs; done`.
