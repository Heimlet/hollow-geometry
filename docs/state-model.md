...
- `lab-ui.js` связывает контролы с командами store. `assembly/change` одной
  командой показывает необходимые компоненты и запускает/перематывает разборку.
...
У Explode две явно выбранные области: фигуры/уровни и компоненты соединений.
