...
export function initialLab() {return {
  rotation:{axis:'diagonal',upAxis:'diagonal',downAxis:'diagonal',upSpeed:34,downSpeed:21,upDirection:1,downDirection:-1,vector:[0,1,0],angle:0,up:0,down:0,speed:34,direction:1,mode:'tradition',running:false},
  ...
  collections:Object.fromEntries(ASSEMBLIES.map(c=>[c.id,{explode:0,direction:0,mirror:false,restore:null}]))
};}
export function labChange(lab,section,patch,id) {
  ...
  for(const [key,value] of Object.entries(patch)) {
    ...
    else if(key==='mode'){if(!['tradition','whole','up','down','counter','independent'].includes(value))fail();}
