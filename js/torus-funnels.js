...
import { createSpiralSurface } from './torus-guide-surface.js';
...
export function createTorusFunnels(parent){
  ...
  const roundedGroup=new THREE.Group();roundedGroup.name='Rounded phi surfaces';group.add(roundedGroup);
  function stroke(points,color,width=1.2,target=roundedGroup){
    const geometry=new THREE.BufferGeometry().setFromPoints(points.map(p=>new THREE.Vector3(...p)));
    const line=new THREE.LineSegments(geometry,new THREE.LineBasicMaterial({color,linewidth:width,transparent:true,opacity:0,depthWrite:false}));target.add(line);return line;
  }
  const guideSurface=createSpiralSurface(group);
  const halves=[1,-1].map(sign=>{
    ...
    const surface=new THREE.Mesh(geometry,material);surface.name=sign>0?'Upper phi funnel':'Lower phi funnel';roundedGroup.add(surface);
    ...
    const rings=[.25,.5,.75,1,1.25,PHI].map(t=>{
      ...
      const line=stroke(points,contact?0xffd277:sign>0?0xb3a2ff:0x8bcfe8,contact?2.6:1.2,contact?group:roundedGroup);
  ...
  const supports=[7,0].map(index=>{
    const seed=ORBIT_SEEDS[index],curve=stroke(Array.from({length:96},(_,i)=>[spiralGuide(seed,i/96),spiralGuide(seed,(i+1)/96)]).flat(),index===7?0xffd277:0x94e6ff,2.6,group);
    ...
    mid.name='Real edge midpoint on the waist';roundedGroup.add(mid);
  ...
  return {group,update(kind,p,frame,anchors,{rounded=true,spiral=true}={}){
    const reveal=funnelReveal(kind,p);group.visible=reveal>0&&(rounded||spiral);if(!group.visible)return;
    roundedGroup.visible=rounded;guideSurface.group.visible=spiral;
    ...
    guideSurface.update(reveal,funnelExtensionReveal(kind,p));
