...
export function check(parent,name,read,write) {
  ...
  let previous;bind(()=>{const v=read();if(v===previous)return;previous=v;input.checked=v===true;input.indeterminate=v==='mixed';}); return input;
}
export function slider(parent,name,min,max,step,read,write,format=v=>String(v)) {
  ...
  let previous;bind(()=>{const v=read();if(v===previous)return;previous=v;input.value=v;output.textContent=format(v);}); return input;
}
export function select(parent,name,options,read,write) {
  ...
  input.addEventListener('change',()=>write(input.value));label.append(input);parent.append(label);let previous;bind(()=>{const v=read();if(v!==previous){previous=v;input.value=v;}});return input;
}
