...
let linkSignature='', sceneLayout=new Map(), statusNode;
...
export function updateLab(dt) {
  ...
  const positions=[];
  if(lab.explode.value===0)sceneLayout.clear();
  ...
  for(const level of levels)for(const [id,object] of [...Object.entries(level.objs),['_metatron_',level.mc]]) {
    ...
    if(lab.explode.scope==='scene'&&object.vis)offset=(sceneLayout.get(key)||new THREE.Vector3()).clone().multiplyScalar(lab.explode.value);
    else if(pack&&object.vis)offset=explodedOffset(pack.members.indexOf(id),pack.members.length,CR*level.scale,conf.explode);
  ...
  if(statusNode){const owners=derivedObjects.filter(o=>o.level===0&&o.object.vis), text=owners.map(o=>`${o.kind==='hull'?'Оболочка':'Пересечение'}: ${o.data?.vertices.length||0} вершин · ${o.data?.faces.length||0} граней${!o.data?.volume?' · нет общего объёма':''}`).join('\n');if(statusNode.textContent!==text)statusNode.textContent=text;}
}
export function fitVisible() {
  ...
  const center=box.getCenter(new THREE.Vector3()),direction=camera.position.clone().sub(controls.target).normalize();controls.target.copy(center);camera.position.copy(center).addScaledVector(direction,30);setViewHeight(box.getSize(new THREE.Vector3()).length()*1.2/Math.min(1,innerWidth/innerHeight));controls.update();
}
